...
//            URL url = new URL("http://192.168.43.118/washmycar/index.php/androidcontroller/get_carwashseeker");
            URL url = new URL("http://192.168.254.128/easyride/index.php/androidcontroller/get_user");
            ...
            JSONArray array = json.getJSONArray("easy_ride_user");
            for(int i=0; i<array.length(); i++){
                JSONObject item = array.getJSONObject(i);
                final String user_name = item.getString("username");
                final String user_pass = item.getString("password");
                final String client_id = item.getString("user_id");
                final String customer_name = item.getString("first_name");
                final String client_last_name = item.getString("last_name");
                final String contact_number = item.getString("contact_number");
                ...
                if(username.equals(user_name) && password.equals(user_pass))
                {
                    ...
                    editor.putString("client_last_name", client_last_name);
